...
impl Scroller {
    ...
    pub fn seek(&mut self, window: &mut window::FixedWindow, addr: addr::Address) {
        let cfg = config::get();
    ...
    pub fn page_up(&mut self, window: &window::FixedWindow) {
        let cfg = config::get();
    ...
    pub fn page_down(&mut self, window: &window::FixedWindow) {
        let cfg = config::get();
